...
void ReceiveLoRaTask(void *pvParameters) {
    while (1) {
        ...
        if (packetSize) {
            ...
            // Tách dữ liệu
            int idx1 = receivedData.indexOf(",");                              // packetID
            int idx2 = receivedData.indexOf(",", idx1 + 1);                     // NODE_ID
            int idx3 = receivedData.indexOf(",", idx2 + 1);                     // status
            int idx4 = receivedData.indexOf(",", idx3 + 1);                     // temp
            int idx5 = receivedData.indexOf(",", idx4 + 1);                     // humid
            int idx6 = receivedData.indexOf(",", idx5 + 1);                     // lux
            int idx7 = receivedData.indexOf(",", idx6 + 1);                     // sound
            ...
            uint32_t packetID = receivedData.substring(0, idx1).toInt();
            ...
            // Lưu thông tin để kiểm tra lần sau
...
// ===== TASK: Gửi dữ liệu lên MQTT (KHÔNG gửi packetID & TTL) =====
void SendMQTTTask(void *pvParameters) {
    while (1) {
        if (xSemaphoreTake(xSemaphore, portMAX_DELAY) == pdTRUE) {
            ...
            jsonData += "\"status\": \"" + status + "\",";  // CHỖ THÊM
...
// ===== LOOP =====
void loop() {
    client.loop();  // Cần gọi liên tục để giữ kết nối MQTT
}
